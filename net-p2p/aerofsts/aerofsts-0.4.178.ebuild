...
src_prepare() {
    ...
    sed 's#uudecode -o /dev/stdout \$0 | tar xzf - -C \$TMP_DIR 2>/dev/null#(cd $TMP_DIR \&\& uudecode "$0" \&\& tar xzf bin -C . 2>/dev/null; rm bin)#' -i aerofsts || die "uudecode sed failed"

    # see http://support.aerofs.com/forums/147816-linux-problems/suggestions/3135916-daemon-exited-with-code-127
    sed 's#^JRE_BASE=.*#JRE_BASE="$JAVA_HOME"#' -i aerofsts || die "JRE_BASE sed failed"
}
